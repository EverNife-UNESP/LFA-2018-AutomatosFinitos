...
public class Validator {
    ...
    public static void loadValidatorToGraph(){
        ...
        for (Vertice vertice : todosOsVertices){
            ...
            Vertex createdVertex = FiniteAutomationApplication.getGraph().addCustomVertex(vertice.getId());
        ...
        for (Aresta aresta : todasAsArestas){
            ...
            Edge createdEdge = FiniteAutomationApplication.getGraph().getOrCreateEdge(aresta.sourceId,aresta.targetId);

            Aresta edgeAresta = createdEdge.getOrCreateAresta(FiniteAutomationApplication.getGraph().getVertex(aresta.getSourceId()));
            edgeAresta.getGrammars().addAll(aresta.getGrammars());
        }
    ...
    public static boolean convertToAFD() {
        ...
        Map<Aresta,List<Character>> valuesToBeRemovedAtTheEnd = new HashMap<Aresta,List<Character>>();

        for (Aresta aresta : todasAsArestas){
            List<Character> selfVerticesGRammars = new ArrayList<>();
            selfVerticesGRammars.addAll(aresta.getGrammars());
        ...
        for (Aresta aresta : todasAsArestas){
            for (Character character : aresta.getGrammars()){
        ...
        do {
            ...
            for (Vertice sourceVertice : interestingVertices){
                ...
                for (Character grammar : allPossibleGrammars){
                    ...
                    for (Vertice splitedVertice : splitedVertices){
                        for (Aresta aresta : splitedVertice.getArestasList()){
                            if (aresta.getGrammars().contains(grammar)){
                                ...
                                //if (!sourceVertice.alreadyChecked) aresta.getGrammars().remove(grammar);
                            }
                        }
                    }
                    ...
                    if (newRule.length() >= 2){
                        ...
                        aresta.getGrammars().add(grammar);
                ...
                /*
                for (Aresta aresta : sourceVertice.getArestasList()){
                    Vertice futureVertice = getVerticeFromID(aresta.getTargetId());
                    if (!interestingVertices.contains(futureVertice) && !newInterestingVertices.contains(futureVertice)){
                        newInterestingVertices.add(futureVertice);
                        needAnotherLoop = true;
                    }
        ...
        for (Aresta aresta : valuesToBeRemovedAtTheEnd.keySet()){
            ...
            for (Character character : grammarsToBeRemoved){
                aresta.getGrammars().remove(character);
            }
        }
        ...
        for (Vertice vertice : markedToBeRemoved){
            FiniteAutomationApplication.getGraph().removeVertex(vertice.getId());
        }
    ...
    public static void removeNullEdges(){
                ...
                for (Aresta aresta : vertice.getArestasList()){
                    ...
                    if (aresta.getSourceId() == aresta.getTargetId()) {
                        if (aresta.getGrammars().contains('ε')) {
                            aresta.getGrammars().removeIf(character -> character == 'ε');
                        }
                        continue; // Ignora caso seja uma aresta apontando para o próprio vertice!
                    }
                    if (aresta.getGrammars().contains('ε')){
                        aresta.getGrammars().forEach(character -> {
                        ...
                        for (int i = 0; i < aresta.getGrammars().size() ; i++){
                            if (aresta.getGrammars().get(i) == 'ε'){
                                aresta.getGrammars().remove(i);
                                break;
                            }
                        }

                        boolean hasMoreConnections = false;
                        if (aresta.getGrammars().size() == 0){
                        ...
                        if (!hasMoreConnections){
                            //Pego todas as arestas do vertice futuro e altero a origem para o vertice anterior
                            for (Aresta inAresta : targetVertice.getArestasList()){
                                inAresta.sourceId = sourceVertice.getId();
                                boolean foundAresta = false;
                                for (Aresta sourceAresta : sourceVertice.getArestasList()){
                                    if (sourceAresta.isSameTransaction(inAresta)){
                                        sourceAresta.getGrammars().addAll(inAresta.getGrammars());
                                        foundAresta = true;
                                        todasAsArestas.remove(inAresta);
                                        break;
                                    }
                                }

                                if (!foundAresta){
                                    sourceVertice.getArestasList().add(inAresta);
                                }
                            }
                        }else {
                        */
                            ...
                            for (Aresta inAresta : targetVertice.getArestasList()){
                                ...
                                for (Aresta sourceAresta : sourceVertice.getArestasList()){
                                    if (sourceAresta.isSameTransaction(newAresta)){
                                        sourceAresta.getGrammars().addAll(newAresta.getGrammars());
                                        foundAresta = true;
                                        break;
                                    }
                                }
                        ...
                        /*
                        for (Aresta reachingAresta : getArestasThatReaches(targetVertice.getId())){
                            reachingAresta.targetId = sourceVertice.getId();
                        }
                        */
        ...
        todasAsArestas.removeIf(aresta -> {
            return (aresta.getGrammars().size() == 0);
        });
