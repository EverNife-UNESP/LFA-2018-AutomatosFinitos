    ...
    public boolean deleteIfInvalid(){
        if (!arestaOne.isValid()){
            ...
            if (arestaTwo.isValid()){
                Edge edge = FiniteAutomationApplication.getGraph().getOrCreateEdge(this.target,this.source);
                this.arestaTwo.getGrammars().forEach(character -> edge.arestaOne.addGrammar(character));
            }
        ...
        if ( isBidiretec() && !arestaTwo.isValid()){
            delete();
            Edge edge = FiniteAutomationApplication.getGraph().getOrCreateEdge(this.source,this.target);
            this.arestaOne.getGrammars().forEach(character -> edge.arestaOne.addGrammar(character));
            FiniteAutomationApplication.getGraph().reloadPage();
            return true;
        }
        return false;
