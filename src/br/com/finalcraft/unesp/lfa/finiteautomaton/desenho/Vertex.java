...
public class Vertex {
    ...
    public void repaint(){
        if (tempEdge != null){
            FiniteAutomationApplication.getGraph().removeAllTempEdges();
            tempEdge = null;
        }
    ...
    public void delete(){
        ...
        for (Edge anEdge : FiniteAutomationApplication.getGraph().getEdges()){
        ...
        for (Edge edge : edgesToBeRemoved){
            FiniteAutomationApplication.getGraph().edges.remove(edge);
        }
        FiniteAutomationApplication.getGraph().vertex.remove(this);
        ...
        vertex.setOnMouseReleased((event) -> {
            switch (MainController.getPressedRadioButton()){
                case EDITAR_TRANSICOES:{
                    ...
                    if (event.getButton().equals(MouseButton.PRIMARY)){
                        for (Node node : FiniteAutomationApplication.getGraph().getGroup().getChildren()){
                            if (node instanceof Circle){
                                ...
                                if (isPointInCircle(circle.getCenterX(),circle.getCenterY(),circle.getRadius(),event.getX(),event.getY())){
                                    ...
                                    Vertex vS = FiniteAutomationApplication.getGraph().getVertex(vIni);
                                    Vertex vT = FiniteAutomationApplication.getGraph().getVertex(vFim);

                                    AddTransitionController.show(vS,vT);

                                /* // Funcinal, but, comentando para testar add_transaction
                                Edge e = new Edge(vS, vT, 0, 0); //desenho
                                FiniteAutomationApplication.graph.addEdge(e);    //desenho
                                e.changeText("\nq1-->q2  : a|b|c|d" +
                                        "\nq2-->q1  : b|2");
                                                                        */
        ...
        vertex.setOnMouseDragged((event) -> {
            switch (MainController.getPressedRadioButton()){
                ...
                case EDITAR_TRANSICOES:{
                    ...
                    //Vertex vS = FiniteAutomationApplication.graph.;
                    /*
                    Vertex tempVertex = new Vertex();
                    tempVertex.setX(this.getX());
                    tempVertex.setY(this.getY());
                    FiniteAutomationApplication.graph.getGroup().getChildren().add(tempVertex.createCircle());
                    ...
                    FiniteAutomationApplication.graph.getGroup().getChildren().add(line);
