 ...
 */
package br.com.finalcraft.unesp.lfa.finiteautomaton.desenho.oldfiles;

/**
 *
 * @author Danilo Medeiros Eler
 *
 * Editado por EverNife
 */
...
/*
    ...
    public boolean deleteIfInvalid(Aresta aresta){
        if (!aresta.isValid()){
            delete(aresta.getSourceId(),aresta.getTargetId());
            clear();
            return true;
        }
        return false;
    ...
    public void calculateAndSetTextOne(){
        ...
        if (bidiretec){
            ...
            for (Character character : arestaOne.getGrammars()){
            ...
            for (Character character : arestaTwo.getGrammars()){
                stringBuilder.append( character + "|");
            }
            stringBuilder.setLength(stringBuilder.length() - 1);
        }else {
    ...
    public void changeText(String aText){
        this.floatingText.setText(aText);
    }
    ...
    public Aresta getArestaOne() {
        return arestaOne;
    }

    public Aresta getArestaTwo() {
        return arestaTwo;
    }
    ...
    public void invertEdge(){
        System.out.println("Invertendo Edge");
        Aresta tempAresta = this.arestaOne;
        tempAresta.getGrammars().clear();
        this.arestaOne = this.arestaTwo;
        this.arestaTwo = tempAresta;

        connect(target.getCircle(),source.getCircle());
    }

    public void delete(int sourceId, int targetId){
        if (bidiretec){
            if (sourceId == this.target.getID() && targetId == this.source.getID()){
                arestaTwo.getGrammars().clear();
            }else {
                invertEdge();
            }
            setBidiretec(false);
        }else {
            FiniteAutomationApplication.getGraph().edges.remove(this);
        }
    ...
    public OldEdge(Vertex source, Vertex target) {
        this.source = source;
        this.target = target;
